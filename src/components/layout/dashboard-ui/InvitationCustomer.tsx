...
const GlassesDesign = dynamic(
  () => import('@/theme/gold/elegan/glasses/main'),
  { ssr: false }
);
const AdatDesign = dynamic(() => import('@/theme/gold/traditional/main'), {
  ssr: false,
});
...
export default function InvitationEditorLayout({ data: initialData }: Props) {
  ...
  const handleSlugChange = (raw: string) => {
    ...
    EditorModels.isSlugTaken(formatted, initialData.invitationId).then(
      (taken) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (eventResults.some((r) => !r.success))
      ...
      <div className='hidden md:block'>
        ...
        <div className='flex h-screen bg-gray-900 overflow-hidden'>
            ...
            {[
              ...
              return (
                ...
                `}
                  ...
                  <span
                    className={`text-[10px] font-medium tracking-wide transition-all duration-300 ${isActive ? 'opacity-100' : 'opacity-60'}`}
                  >
                    {tab.label}
                  </span>
                </button>
          ...
          <section className='hidden md:flex flex-1 items-center justify-center bg-gray-950/50 relative overflow-hidden'>
            ...
            />
            <DevicePreview>{renderTheme(data, true)}</DevicePreview>
          </section>
        </div>
      </div>
