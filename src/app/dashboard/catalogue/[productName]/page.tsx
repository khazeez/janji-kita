...
export default function DashboardProductDetail({ params }: PageProps) {
    ...
    const checkFavorite = async () => {
      try {
        ...
        if (res.ok) {
          ...
          const ids = new Set(
            (result.data || []).map((f: any) => f.PRODUCT.PRODUCT_ID)
          );
  ...
  const handleUseDesign = async () => {
    ...
    if (session) {
      ...
      if (count != null && count >= 3) {
        toast.error(
          `Kamu sudah memiliki ${count} undangan dalam status draft. Selesaikan atau aktifkan terlebih dahulu sebelum membuat undangan baru.`
        );
    ...
    router.push(
      `/create/${selectedItem.productId}${withPhoto ? '' : '?variant=no-photo'}`
    );
  };
    ...
    return (
      <div className='min-h-[500px] flex flex-col items-center justify-center text-center p-8 bg-gray-800/30 rounded-3xl border border-gray-700/50 max-w-lg mx-auto'>
        ...
        <p className='text-gray-400 mb-8 max-w-sm'>
          {error || 'Produk tidak ditemukan di database kami.'}
        </p>
        <button
          onClick={handleBack}
  ...
  const features = (
    Array.isArray(selectedItem?.features) ? selectedItem?.features : []
  ) as string[];
          ...
          <div className='grid sm:grid-cols-2 gap-4'>
            <div className='p-5 bg-gray-800/40 rounded-3xl border border-gray-700/50'>
              ...
              <p className='text-gray-400 text-sm leading-relaxed'>
                Selamanya. Undangan tetap dapat diakses meskipun acara telah
                selesai.
              </p>
            </div>
            <div className='p-5 bg-gray-800/40 rounded-3xl border border-gray-700/50'>
              ...
              <p className='text-gray-400 text-sm leading-relaxed'>
                Data tamu aman dan notifikasi RSVP dikirim langsung ke Anda.
              </p>
            </div>
            <div className='p-5 bg-gray-800/40 rounded-3xl border border-gray-700/50'>
              <div className='flex items-center gap-3 mb-3'>
              ...
              <p className='text-gray-400 text-sm leading-relaxed'>
                Tambahkan album foto prewedding dan momen spesial Anda.
              </p>
            </div>
            <div className='p-5 bg-gray-800/40 rounded-3xl border border-gray-700/50'>
              <div className='flex items-center gap-3 mb-3'>
              ...
              <p className='text-gray-400 text-sm leading-relaxed'>
                Sesuaikan warna, font, dan konten sesuai keinginan Anda.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className='lg:col-span-5 space-y-6'>
          ...
          <div className='bg-gray-800/50 backdrop-blur-xl border border-gray-700/50 rounded-[2rem] p-6 sm:p-8 space-y-8 shadow-2xl relative overflow-hidden'>
            ...
            <div className='space-y-3'>
              ...
              <p className='text-gray-400 text-sm leading-relaxed'>
                Sesuaikan dengan kebutuhan undangan Anda.
              </p>
            </div>
            ...
            <div className='space-y-3'>
              ...
              >
                ...
                <div className='text-right'>
                  {!withPhoto && (
                    <div className='text-[10px] font-black text-pink-500 uppercase mb-1.5'>
                      Terpilih
                    </div>
                  )}
                  <p
                    className={cn(
                      'text-xl font-black',
                      !withPhoto ? 'text-pink-500' : 'text-gray-400'
                    )}
                  >
